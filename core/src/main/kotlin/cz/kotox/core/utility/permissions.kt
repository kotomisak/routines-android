...
abstract class PermissionManager {
	...
	fun requestLocationPermissions(grantedCallback: (grantedPermissions: String) -> Unit = {}, deniedCallback: (deniedPermissions: String) -> Unit = {}) {
		if (!checkPermission(ACCESS_FINE_LOCATION)) {
			requestPermission(SinglePermissionRequest(ACCESS_FINE_LOCATION, grantedCallback, deniedCallback))
		}
	}

	fun requestStoragePermissions(grantedCallback: (grantedPermissions: String) -> Unit = {}, deniedCallback: (deniedPermissions: String) -> Unit = {}) {
		if (!checkPermission(WRITE_EXTERNAL_STORAGE)) {
			requestPermission(SinglePermissionRequest(WRITE_EXTERNAL_STORAGE, grantedCallback, deniedCallback))
		}
	}

	fun requestRecordAudioPermissions(grantedCallback: (grantedPermissions: String) -> Unit = {}, deniedCallback: (deniedPermissions: String) -> Unit = {}) {
